...
void animate_stuff(Board *b)
{
    ALLEGRO_DISPLAY *disp = al_get_current_display();
    TiledBlock *t = &b->panel;
    ...
    for (i=0; i<t->sb; i++)
    {
        float angle = (float)i / (float)t->sb * 2.0f * ALLEGRO_PI;
        t->b[i]->x += angle;
        t->b[i]->y += angle; 
        for (int j = 0; j < t->b[i]->sb; j++)
        {
            t->b[i]->b[j]->x += sz*sin(angle);
            t->b[i]->b[j]->y += sz*cos(angle);
        }

        draw_TiledBlock(t->b[i], t->x, t->y);
    }

    al_flip_display();
}

void move_TiledBlock_bmp(TiledBlock *t, int x, int y){
    int i;
    if (t->sb == 0)
    {  
        if (t->bmp)
    ...
    for (i=0; i<t->sb; i++)
    {
        move_TiledBlock_bmp(t->b[i], t->x + x, t->y + y);
    }
    draw_TiledBlock(t, x, y);
}
...
void animate_win(Board *b) {
    ...
    for(k=0;k<b->n*b->h; k++){
        ...
        //al_rest(max(0.1, 0.3*(1 - sqrt((float) k / (b->h*b->n)))));
    }
}
                ...
                case ALLEGRO_EVENT_KEY_CHAR:
                    ...
                    switch(ev.keyboard.keycode){
                        ...
                        case ALLEGRO_KEY_SPACE:
                            ...
                            g.time = 1;
                            switch_solve_puzzle(&g, &b);
                            while(1) { animate_stuff(&b); al_rest(0.02); }                         
                                break;
                    }
                    break;
